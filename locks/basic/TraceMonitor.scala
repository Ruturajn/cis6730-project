...
/*
  prop locksBasic : Forall t . Forall l . ((sleep(t) -> ![acq(t,l),rel(t,l))) & (acq(t,l) -> !exists s . @ [acq(s,l),rel(s,l))) & (rel(t,l) -> @ [acq(t,l),rel(t,l)))) 
*/

class Formula_locksBasic(monitor: Monitor) extends Formula(monitor) {
          
  override def evaluate(): Boolean = {
    // assignments1 (leaf nodes that are not rule calls):
      now(5) = build("sleep")(V("t"))
      now(8) = build("acq")(V("t"),V("l"))
      now(9) = build("rel")(V("t"),V("l"))
      now(11) = build("acq")(V("t"),V("l"))
      now(16) = build("acq")(V("s"),V("l"))
      now(17) = build("rel")(V("s"),V("l"))
      now(19) = build("rel")(V("t"),V("l"))
      now(22) = build("acq")(V("t"),V("l"))
      now(23) = build("rel")(V("t"),V("l"))
    ...
    // assignments5 (main formula excluding leaf nodes):
      now(7) = now(8).or(now(9).not().and(pre(7)))
      now(6) = now(7).not()
      now(4) = now(5).not().or(now(6))
      now(15) = now(16).or(now(17).not().and(pre(15)))
      now(14) = pre(15)
      now(13) = var_s.seen.and(now(14)).exist(var_s.quantvar)
      now(12) = now(13).not()
      now(10) = now(11).not().or(now(12))
      now(3) = now(4).and(now(10))
      now(21) = now(22).or(now(23).not().and(pre(21)))
      now(20) = pre(21)
      now(18) = now(19).not().or(now(20))
      now(2) = now(3).and(now(18))
      now(1) = now(2).forAll(var_l.quantvar)
      now(0) = now(1).forAll(var_t.quantvar)
  ...
  val var_t :: var_l :: var_s :: Nil = declareVariables(("t",false), ("l",false), ("s",true))(0)

  varsInRelations = Set()
  val indices: List[Int] = List(20,21,14,15,7)

  pre = Array.fill(24)(bddGenerator.False)
  now = Array.fill(24)(bddGenerator.False)

  txt = Array(
    "Forall t . Forall l . ((sleep(t) -> ![acq(t,l),rel(t,l))) & (acq(t,l) -> !exists s . @ [acq(s,l),rel(s,l))) & (rel(t,l) -> @ [acq(t,l),rel(t,l))))",
      "Forall l . ((sleep(t) -> ![acq(t,l),rel(t,l))) & (acq(t,l) -> !exists s . @ [acq(s,l),rel(s,l))) & (rel(t,l) -> @ [acq(t,l),rel(t,l))))",
      "(sleep(t) -> ![acq(t,l),rel(t,l))) & (acq(t,l) -> !exists s . @ [acq(s,l),rel(s,l))) & (rel(t,l) -> @ [acq(t,l),rel(t,l)))",
      "(sleep(t) -> ![acq(t,l),rel(t,l))) & (acq(t,l) -> !exists s . @ [acq(s,l),rel(s,l)))",
...
class PropertyMonitor extends Monitor {
  def eventsInSpec: Set[String] = Set("sleep","acq","rel")

  formulae ++= List(new Formula_locksBasic(this))
}
