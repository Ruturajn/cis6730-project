...
class Formula_file(monitor: Monitor) extends Formula(monitor) {
          
  override def evaluate(): Boolean = {
    // assignments1 (leaf nodes that are not rule calls):
      now(2) = build("close")(V("f"))
      now(6) = build("open")(V("f"),V("m"))
      now(7) = build("close")(V("f"))
    ...
    // assignments5 (main formula excluding leaf nodes):
      ...
      now(4) = pre(5)
      now(3) = var_m.seen.and(now(4)).exist(var_m.quantvar)
      now(1) = now(2).not().or(now(3))
      now(0) = var_f.seen.imp(now(1)).forAll(var_f.quantvar)
  ...
  val var_f :: var_m :: Nil = declareVariables(("f",true), ("m",true))(0)

  varsInRelations = Set()
  val indices: List[Int] = List(4,5)
...
class PropertyMonitor extends Monitor {
  def eventsInSpec: Set[String] = Set("close","open")

  formulae ++= List(new Formula_file(this))
}
