...
/*
  prop telemetry2 : forall x . closed(x) -> !telem(x) 
  where
    closed(x) := (!@ true & !toggle(x)) | (@ closed(x) & !toggle(x)) | (@ open(x) & toggle(x))
    open(x) := (@ open(x) & !toggle(x)) | (@ closed(x) & toggle(x))

*/

class Formula_telemetry2(monitor: Monitor) extends Formula(monitor) {
          
  override def evaluate(): Boolean = {
    // assignments1 (leaf nodes that are not rule calls):
      now(4) = build("telem")(V("x"))
      now(12) = build("toggle")(V("x"))
      now(17) = build("toggle")(V("x"))
      now(21) = build("toggle")(V("x"))
      now(27) = build("toggle")(V("x"))
      now(31) = build("toggle")(V("x"))
    // assignments2 (rule nodes excluding what is below @ and excluding leaf nodes):
      now(9) = pre(10)
      now(8) = now(9).not()
      now(11) = now(12).not()
      now(7) = now(8).and(now(11))
      now(14) = pre(15)
      now(16) = now(17).not()
      now(13) = now(14).and(now(16))
      now(6) = now(7).or(now(13))
      now(19) = pre(20)
      now(18) = now(19).and(now(21))
      now(5) = now(6).or(now(18))
      now(24) = pre(25)
      now(26) = now(27).not()
      now(23) = now(24).and(now(26))
      now(29) = pre(30)
      now(28) = now(29).and(now(31))
      now(22) = now(23).or(now(28))
    ...
    // assignments4 (the rest of rules that are below @ and excluding leaf nodes):
      now(10) = bddGenerator.True
    // assignments5 (main formula excluding leaf nodes):
      now(3) = now(4).not()
      now(1) = now(2).not().or(now(3))
      now(0) = var_x.seen.imp(now(1)).forAll(var_x.quantvar)
  ...
  val var_x :: Nil = declareVariables(("x",true))(0)

  varsInRelations = Set()
  val indices: List[Int] = List(29,24,19,14,9)

  pre = Array.fill(32)(bddGenerator.False)
  now = Array.fill(32)(bddGenerator.False)
  ...
  txt = Array(
    "forall x . closed(x) -> !telem(x)",
      ...
      "!telem(x)",
      "telem(x)",
      "(!@ true & !toggle(x)) | (@ closed(x) & !toggle(x)) | (@ open(x) & toggle(x))",
      "(!@ true & !toggle(x)) | (@ closed(x) & !toggle(x))",
      "!@ true & !toggle(x)",
      "!@ true",
      "@ true",
      "true",
...
class PropertyMonitor extends Monitor {
  def eventsInSpec: Set[String] = Set("telem","toggle")

  formulae ++= List(new Formula_telemetry2(this))
}
