...
/*
  prop MoneyReqAdv : forall r . req(r) -> P exists s . [req(r),send(s,r)) 
*/

class Formula_MoneyReqAdv(monitor: Monitor) extends Formula(monitor) {
          
  override def evaluate(): Boolean = {
    // assignments1 (leaf nodes that are not rule calls):
      now(2) = build("req")(V("r"))
      now(6) = build("req")(V("r"))
      now(7) = build("send")(V("s"),V("r"))
    ...
    // assignments5 (main formula excluding leaf nodes):
      ...
      now(4) = var_s.seen.and(now(5)).exist(var_s.quantvar)
      now(3) = now(4).or(pre(3))
      now(1) = now(2).not().or(now(3))
      now(0) = var_r.seen.imp(now(1)).forAll(var_r.quantvar)
  ...
  val var_r :: var_s :: Nil = declareVariables(("r",true), ("s",true))(0)
  ...
  txt = Array(
    "forall r . req(r) -> P exists s . [req(r),send(s,r))",
      "req(r) -> P exists s . [req(r),send(s,r))",
      "req(r)",
      "P exists s . [req(r),send(s,r))",
      "exists s . [req(r),send(s,r))",
      "[req(r),send(s,r))",
      "req(r)",
      "send(s,r)"
  )
...
class PropertyMonitor extends Monitor {
  def eventsInSpec: Set[String] = Set("req","send")

  formulae ++= List(new Formula_MoneyReqAdv(this))
}
